...
class LawnRobot:
    ...
    # Checks if the robot has reached the top or bottom point of the lawn
    if ((self.pos_y == lawn_height) or (self.pos_y == 0)):

        # Increases robot's x-position 
        self.pos_x += self.width # mm
        
        # Prints position to terminal (for testing)
        print("Pos x: ", self.pos_x)

        # Checks if the robot has reached the last column of grass to mow and exits program if true
        if ((self.pos_x >= lawn_width) or (self.cols_mowed == int(lawn_width/self.width))):
            print("Columns mowed: ", self.cols_mowed)
            print("Mowed ", (self.cols_mowed*self.pos_x*lawn_height)/1e3, " square meters")
            self.is_done = True
            finish_program()
            return
    ...
    def begin_new_column(self):
        if ((self.cols_rowed%2) == 0): # Clockwise 180-degree turn if number of columns mowed is even
            db.turn(90)
    ...
    def interpolate_distance_y(self, hypotenuse, angle):
        ...
        angle_in_rad = ((math.pi*angle)/180) # Angle in radians

        # Gets length of the distance moved on the y-axis during diagonal movement
        adj_cathetus = hypotenuse * math.cos(angle_in_rad) # Adjacent cathetus, aka 'hosliggende katet'

        # Increases y-position with the distance moved on the y-axis only 
        self.pos_y += (adj_cathetus * 2) + hypotenuse

        # Returns the difference between distance robot has driven in total and distance moved on y-axis
        return db.distance() - self.pos_y 
    ...
    def mow_lawn(self, lawn_height, lawn_width):
        ...
        while (uss.distance() <= 370 and obstacle_met == False):
            db.drive(int(drive_speed/2), self.turn_rate) # Slows down in front of obstacle
        ...
        if (self.pos_y < lawn_height and obstacle_met):
            ...
            check_column_status(lawn_height, lawn_width)
